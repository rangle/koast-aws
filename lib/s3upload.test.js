  ...
  it('Should upload a file and retrieve it.', function(done) {
        ...
        respond: function (req, res, s3keys) {
          ...
          for (var property in s3keys) {
            if (req.files.hasOwnProperty(property)) {
              var newUrl = 'https://s3.amazonaws.com/' + bucket +'/' + s3keys[property];
              respVal.push({ url: newUrl });
            }
          }
    ...
    supertest(app)
      ...
      .end(function(err, res) {
        ...
        expect(url).to.equal('https://s3.amazonaws.com/' + bucket + '/' + 'koast.txt');

        //this URL may break.
        supertest('https://s3.amazonaws.com/' + bucket)
          .get('/koast.txt')
          .expect(200)
          .end(function(err, res) {
    ...
    var awsconf = {
      ...
      s3: {
        ...
        respond: function (req, res, s3keys) {
          ...
          for (var property in s3keys) {
            if (req.files.hasOwnProperty(property)) {
              respVal[property] = {url: 'https://s3.amazonaws.com/' + bucket +'/' + s3keys[property]};
            }
          }
